...
// Fri Oct 24 12:26:36 2014 by ROOT version 5.34/20
// from TTree outputTree/outputTree
// found on file: /data/public_DQM_plots/343/dqmPlotstotal.root
//////////////////////////////////////////////////////////

#ifndef DQMpedestals_h
#define DQMpedestals_h
...
public :
   ...
   DQMpedestals(TTree *tree=0);
   virtual ~DQMpedestals();
...
#ifdef DQMpedestals_cxx
DQMpedestals::DQMpedestals(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("foo.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("foo.root");
      }
...
DQMpedestals::~DQMpedestals()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t DQMpedestals::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t DQMpedestals::LoadTree(Long64_t entry)
{
...
void DQMpedestals::Init(TTree *tree)
{
...
void DQMpedestals::Show(Long64_t entry)
{
...
Int_t DQMpedestals::Cut(Long64_t entry)
{
